...
try:
    if not os.path.exists("generated/ham/upfeaturesFreq"):
        os.makedirs("generated/ham/upfeaturesFreq")

except OSError:
    print('Error: Creating Folder1')

try:
    if not os.path.exists("generated/ham/lowfeaturesFreq"):
        os.makedirs("generated/ham/lowfeaturesFreq")
...
try:
    if not os.path.exists("generated/ham/left"):
        os.makedirs("generated/ham/left")

except OSError:
    print('Error: Creating Folder1')

try:
    if not os.path.exists("generated/ham/center"):
        os.makedirs("generated/ham/center")

except OSError:
    print('Error: Creating Folder1')

try:
    if not os.path.exists("generated/ham/right"):
        os.makedirs("generated/ham/right")
...
try:
    if not os.path.exists("generated/ham/upfeatures"):
        os.makedirs("generated/ham/upfeatures")

except OSError:
    print('Error: Creating Folder1')

try:
    if not os.path.exists("generated/ham/lowfeatures"):
        os.makedirs("generated/ham/lowfeatures")

except OSError:
    print('Error: Creating Folder1')

try:
    if not os.path.exists("generated/ham/sms"):
        os.makedirs("generated/ham/sms")
...
var.set("<-- Click on Browse File Button to select HAM file.\n it must be in .txt format")
...
def removeUnwanterSpace():
    ...
    showSMS = "The given ham: ", originalSMS, " with length: ", len(originalSMS)
...
var = StringVar()
extractLabel = Label(main2, textvariable=var, relief=FLAT)
var.set("<-- Click on Remove Unwanted Character(s) Button to remove unwanted character(s) from HAM\n and perform the 1D-TP transformation.")
...
def controlGenTextUTF8():
    import genTextUTF8
    print("The UTF-8 values for the ham is: ", genTextUTF8.genTextUTF8(originalSMS), " \n")
    fwriterham = open("generated/ham/sms/utf.txt", "a+")

    aa = "The UTF-8 values for the ham is: ", genTextUTF8.genTextUTF8(originalSMS)
    fwriterham.write(str(aa) + '\n')
    fwriterham.close()
    ...
    titleIt = "HAM Message Unicodes Signal"

    plt.title(titleIt)
    plt.ylabel("UTF-8 values of characters")
    plt.savefig('generated/HamUTF.png')
...
def upFeaturesOf1DT(upFeaturesList=None):
    ...
    print("The upFeatures 1D-TP signals for the ham is: ", upFeaturesList1, " \n")
    aa = "The upFeatures 1D-TP signals for the ham is: ", upFeaturesList1, " \n"
    text = Text(main2, height=2, width=50)
    text.insert(INSERT, str(aa).replace("'", "").replace("{", "").replace("}", "").replace("(", "").replace(")", ""))
    text.place(x=423, y=340)
    import plotting_graph_output_UPFeatures_1D_TP_signals_SMS

    titleIt = "1D-TP signal(Up) for HAM Message at ρ ".replace("'", "") + str(int(textP.get('1.0', END))) + " and β = " + str(int(textB.get('1.0', END)))
    plotting_graph_output_UPFeatures_1D_TP_signals_SMS.plotting_graph_output_UPFeatures_1D_TP_signals_SMS(upFeaturesList1, titleIt, 'ham')


def lowFeaturesOf1DT():
    global upFeaturesList1, lowFeaturesList1
    # upFeaturesList = upFeaturesList
    print("The lowFeatures 1D-TP signals for the ham is: ", lowFeaturesList1, " \n")
    aa = "The lowFeatures 1D-TP signals for the ham is: ", lowFeaturesList1, " \n"

    text = Text(main2, height=2, width=50)
    text.insert(INSERT, str(aa).replace("'", "").replace("{", "").replace("}", "").replace("(", "").replace(")", ""))
    text.place(x=423, y=390)
    titleIt = "1D-TP signal(Low) for HAM Message at ρ = " + str(int(textP.get('1.0', END))) + " and β = " + str(int(textB.get('1.0', END)))
    import plotting_graph_show_output_lowFeatures_1D_TP_signals_SMS
    plotting_graph_show_output_lowFeatures_1D_TP_signals_SMS.plotting_graph_show_output_lowFeatures_1D_TP_signals_SMS(lowFeaturesList1, titleIt, 'ham' )


def upHistValues_upHistFreqOf1DT():
    global upHistValues1, upHistFreq1
    # upFeaturesList = upFeaturesList
    print("The Up Features 1D-TP signals unique Histogram Values for the ham is: ", upHistValues1,
          " \nwith frequency values: ", upHistFreq1, " \n")
    aa = "The Up Features 1D-TP signals unique Histogram Values for the ham is: ", upHistValues1, " \nwith frequency values: ", upHistFreq1, " \n"
    ...
    titleIt = "1D-TP Histogram(Up Features) for HAM Message at ρ = " + str(int(textP.get('1.0', END))) + " and β = " + str(int(textB.get('1.0', END)))
    import plotting_graph_show_output_Histogram_UPFeatures_1D_TP_signals_SMS
    plotting_graph_show_output_Histogram_UPFeatures_1D_TP_signals_SMS.plotting_graph_show_output_Histogram_UPFeatures_1D_TP_signals_SMS(upHistValues1, upHistFreq1, titleIt, 'ham')
...
def lowHistValues_lowHistFreqOf1DT():
    ...
    # upFeaturesList = upFeaturesList
    print("The Up Features 1D-TP signals unique Histogram Values for the ham is: ", lowHistValues1,
          " \nwith frequency values: ", lowHistFreq1, " \n")
    aa = "The Up Features 1D-TP signals unique Histogram Values for the ham is: ", lowHistValues1, " \nwith frequency values: ", lowHistFreq1, " \n"

    text = Text(main2, height=3, width=50)
    text.insert(INSERT, str(aa).replace("'", "").replace("{", "").replace("}", "").replace("(", "").replace(")", "").replace(",", ""))
    text.place(x=428, y=510)
    titleIt = "1D-TP Histogram(Low Features) for HAM Message at ρ = " + str(int(textP.get('1.0', END))) + " and β = " + str(int(textB.get('1.0', END)))
    import plotting_graph_show_output_Histogram_LOWFeatures_1D_TP_signals_SMS
    plotting_graph_show_output_Histogram_LOWFeatures_1D_TP_signals_SMS.plotting_graph_show_output_Histogram_LOWFeatures_1D_TP_signals_SMS(lowHistValues1, lowHistFreq1, titleIt, 'ham')
