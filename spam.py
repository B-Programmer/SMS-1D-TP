...
try:
    if not os.path.exists("generated/spam/upfeaturesFreq"):
        os.makedirs("generated/spam/upfeaturesFreq")

except OSError:
    print('Error: Creating Folder1')

try:
    if not os.path.exists("generated/spam/lowfeaturesFreq"):
        os.makedirs("generated/spam/lowfeaturesFreq")

except OSError:
    print('Error: Creating Folder1')

try:
    if not os.path.exists("generated/spam/bestObjective"):
        os.makedirs("generated/spam/bestObjective")
...
try:
    if not os.path.exists("generated/spam/left"):
        os.makedirs("generated/spam/left")

except OSError:
    print('Error: Creating Folder1')

try:
    if not os.path.exists("generated/spam/center"):
        os.makedirs("generated/spam/center")

except OSError:
    print('Error: Creating Folder1')

try:
    if not os.path.exists("generated/spam/right"):
        os.makedirs("generated/spam/right")
...
try:
    if not os.path.exists("generated/spam/upfeatures"):
        os.makedirs("generated/spam/upfeatures")

except OSError:
    print('Error: Creating Folder1')

try:
    if not os.path.exists("generated/spam/lowfeatures"):
        os.makedirs("generated/spam/lowfeatures")

except OSError:
    print('Error: Creating Folder1')

try:
    if not os.path.exists("generated/spam/sms"):
        os.makedirs("generated/spam/sms")
...
var.set("<-- Click on Browse File Button to select SPAM file.\n it must be in .txt format")
...
def removeUnwanterSpace():
    ...
    showSMS = "The given spam: ", originalSMS, " with length: ", len(originalSMS)
...
var = StringVar()
extractLabel = Label(main2, textvariable=var, relief=FLAT)
var.set("<-- Click on Remove Unwanted Character(s) Button to remove unwanted character(s) from SMS and\n perform the 1D-TP transformation.")
...
def controlGenTextUTF8():
    import genTextUTF8
    print("The UTF-8 values for the spam is: ", genTextUTF8.genTextUTF8(originalSMS), " \n")
    fwriterspam = open("generated/spam/sms/utf.txt", "a+")
    aa = "The UTF-8 values for the spam is: ", genTextUTF8.genTextUTF8(originalSMS)
    fwriterspam.write(str(aa) + '\n')
    fwriterspam.close()
    ...
    titleIt = "SPAM Message Unicodes Signal"

    plt.title(titleIt)
    plt.ylabel("UTF-8 values of characters")
    plt.savefig('generated/SpamUTF.png')
...
def upFeaturesOf1DT(upFeaturesList=None):
    ...
    print("The upFeatures 1D-TP signals for the spam is: ", upFeaturesList1, " \n")
    aa = "The upFeatures 1D-TP signals for the spam is: ", upFeaturesList1, " \n"
    text = Text(main2, height=2, width=50)
    text.insert(INSERT, str(aa).replace("'", "").replace("{", "").replace("}", "").replace("(", "").replace(")", ""))
    text.place(x=423, y=340)
    import plotting_graph_output_UPFeatures_1D_TP_signals_SMS

    titleIt = "1D-TP signal(Upper) for SPAM Message at ρ ".replace("'", "") + str(int(textP.get('1.0', END))) + " and β = " + str(int(textB.get('1.0', END)))
    plotting_graph_output_UPFeatures_1D_TP_signals_SMS.plotting_graph_output_UPFeatures_1D_TP_signals_SMS(upFeaturesList1, titleIt, 'spam')


def lowFeaturesOf1DT(upFeaturesList=None):
    global upFeaturesList1, lowFeaturesList1
    # upFeaturesList = upFeaturesList
    print("The lowFeatures 1D-TP signals for the spam is: ", lowFeaturesList1, " \n")
    aa = "The lowFeatures 1D-TP signals for the spam is: ", lowFeaturesList1, " \n"

    text = Text(main2, height=2, width=50)
    text.insert(INSERT, str(aa).replace("'", "").replace("{", "").replace("}", "").replace("(", "").replace(")", ""))
    text.place(x=423, y=390)
    titleIt = "1D-TP signal(Low) for SPAM Message at ρ = " + str(int(textP.get('1.0', END))) + " and β = " + str(int(textB.get('1.0', END)))
    import plotting_graph_show_output_lowFeatures_1D_TP_signals_SMS
    plotting_graph_show_output_lowFeatures_1D_TP_signals_SMS.plotting_graph_show_output_lowFeatures_1D_TP_signals_SMS(lowFeaturesList1, titleIt, 'spam' )


def upHistValues_upHistFreqOf1DT():
    global upHistValues1, upHistFreq1
    # upFeaturesList = upFeaturesList
    print("The Up Features 1D-TP signals unique Histogram Values for the spam is: ", upHistValues1,
          " \nwith frequency values: ", upHistFreq1, " \n")
    aa = "The Up Features 1D-TP signals unique Histogram Values for the spam is: ", upHistValues1, " \nwith frequency values: ", upHistFreq1, " \n"
    ...
    titleIt = "1D-TP Histogram(Up Features) for SPAM Message at ρ = " + str(int(textP.get('1.0', END))) + " and β = " + str(int(textB.get('1.0', END)))
    import plotting_graph_show_output_Histogram_UPFeatures_1D_TP_signals_SMS
    plotting_graph_show_output_Histogram_UPFeatures_1D_TP_signals_SMS.plotting_graph_show_output_Histogram_UPFeatures_1D_TP_signals_SMS(upHistValues1, upHistFreq1, titleIt, 'spam')
...
def lowHistValues_lowHistFreqOf1DT():
    ...
    # upFeaturesList = upFeaturesList
    print("The Up Features 1D-TP signals unique Histogram Values for the spam is: ", lowHistValues1,
          " \nwith frequency values: ", lowHistFreq1, " \n")
    aa = "The Up Features 1D-TP signals unique Histogram Values for the spam is: ", lowHistValues1, " \nwith frequency values: ", lowHistFreq1, " \n"

    text = Text(main2, height=3, width=50)
    text.insert(INSERT, str(aa).replace("'", "").replace("{", "").replace("}", "").replace("(", "").replace(")", "").replace(",", ""))
    text.place(x=428, y=510)
    titleIt = "1D-TP Histogram(Low Features) for SPAM Message at ρ = " + str(int(textP.get('1.0', END))) + " and β = " + str(int(textB.get('1.0', END)))
    import plotting_graph_show_output_Histogram_LOWFeatures_1D_TP_signals_SMS
    plotting_graph_show_output_Histogram_LOWFeatures_1D_TP_signals_SMS.plotting_graph_show_output_Histogram_LOWFeatures_1D_TP_signals_SMS(lowHistValues1, lowHistFreq1, titleIt, 'spam')
